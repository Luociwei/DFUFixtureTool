...
def test_object_method_register_with_prefix(dispatch):
    ...
    with pytest.raises(MethodNotFoundError):
        assert dispatch.get_method('myprefix_foo1')

    with pytest.raises(MethodNotFoundError):
        assert dispatch.get_method('wrongprefix_foo1')
    ...
    assert dispatch.get_method('myprefix_foo2') == f.foo2
    assert dispatch.get_method('myprefix_baz') == f.foo3
    all_methods = dispatch.all_methods()
    assert '' in all_methods
    assert 'myprefix_' in all_methods
    found_foo1 = False
    found_foo2 = False
    found_foo3 = False
    found_baz = False
    for rpc in all_methods['myprefix_']:
        if rpc['name'] == 'foo1':
            found_foo1 = True
        elif rpc['name'] == 'foo2':
            found_foo2 = True
