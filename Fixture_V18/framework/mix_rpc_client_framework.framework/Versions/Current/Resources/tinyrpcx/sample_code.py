...
class utility(object):
    ...
    @public('sleep')
    def test_delay(self, second, timeout_ms=None):
        now = time.time()
...
def sample_rpc_service_discovery(client):
    ...
    # the following rpc call are calling functions in sub-instance; not supported for the moment.
    ret = client_driver.bus_fun()
    print 'ret of client.bus_fun(): ', ret
    assert ret == 'calling driver.bus.fun()'

    ret = client_driver.axi_fun()
    print 'ret of client.axi_fun(): ', ret
    assert ret == 'calling axi.fun()'

    ret = client_driver.bus_axi_fun()
    print 'ret of client.bus_axi_fun(): ', ret
    assert ret == 'calling axi.fun()'
...
def sample_check_server_accessibility(client_yes):
    ...
    ret_yes = client_yes.server_rpc_version('9.0.0')
    assert type(ret_yes) is unicode
    assert ret_yes >= '1.0.0'
    try:
