...
void deleteFixtureLog(void* controller)
{
    @synchronized (@"com.writeFixtureLog.lock")
    {
        NSDateFormatter* DateFomatter = [[NSDateFormatter alloc] init];
        [DateFomatter setDateFormat:@"yyyy_MM_dd_HH_mm_ss_SSS "];
        NSString* timeFlag = [DateFomatter stringFromDate:[NSDate date]];
        
        NSString *filePath = @"/vault/Atlas/FixtureLog/SunCode/SCFixture_Command.txt";
        NSString *newfilePath = @"";
        
        newfilePath = [newfilePath stringByAppendingFormat:@"/vault/FixtureLog/SunCode/SCFixture_Command_%@.txt",timeFlag];
        ...
        if (isExist)
        {
            ...
            NSString *ret = [fixture WriteReadString:@"version.version()" atSite:1 timeOut:6000];
            usleep(10);
            writeFixtureLog(ret);

            NSString *apattern = @"MIX_FW_PACKAGE\"\\s\\=\\s(\\d+)";
            ...
            usleep(10);
            writeFixtureLog([NSString stringWithFormat:@"Xaiver_version :  %@\r\n",Version]);
            Xavier_Version = [NSString stringWithFormat:@"Lib_v3.0.18_XavierFW_%@",Version];
...
const char * const get_serial_number(void* controller)
{
    ...
    RPCController *fixture = (__bridge RPCController *)controller;
    NSString  *serial_number =[NSString stringWithFormat:@"%@()",[cmd objectForKey:kSERIAL]];
    NSString *ret = [fixture WriteReadString:serial_number atSite:1 timeOut:3000];
//    writeFixtureLog([NSString stringWithFormat:@"serialnumber_from_xavier: %@ \r\n",ret]);
    NSString *SN = @"";
    NSString *apattern = @"ACK\\(\"(SC\\d+\\D+\\-\\d+)\";DONE\\)";
    NSRegularExpression *regular = [[NSRegularExpression alloc] initWithPattern:apattern options:NSRegularExpressionCaseInsensitive error:nil];
    NSArray *results = [regular matchesInString:ret options:0 range:NSMakeRange(0,ret.length)];
...
//        return [obj UTF8String];
        writeFixtureLog(Xavier_Version);
        return [Xavier_Version UTF8String];
//    }
//    else
//    {
...
bool is_board_detected(void* controller, int site)
{
    ...
    RPCController *fixture = (__bridge RPCController *)controller;
    NSString  *detectcmd =[NSString stringWithFormat:@"%@()",[cmd objectForKey:kFIXTUREUUTDETECT]];
...
bool is_fan_ok(void* controller, int site)
{
    ...
    RPCController *fixture = (__bridge RPCController *)controller;
    NSString *fan_get = [NSString stringWithFormat:@"%@(%d)",[cmd objectForKey:kFIXTUREFANSPEEDGET],site];
    int pre_speed = [[fixture WriteReadString:fan_get atSite:site-1 timeOut:6000] intValue];
    writeFixtureLog([NSString stringWithFormat:@"[cmd] %@, [pre speed result] %d",fan_get,pre_speed]);
    
    NSString *fan_ok_set = [NSString stringWithFormat:@"%@(4000,%d)",[cmd objectForKey:kFIXTUREFANSPEEDSET],100-90];
    bool is_ok = false;
    for (int i = 0; i < 3; i++)
    {
        [fixture WriteReadString:fan_ok_set atSite:site-1 timeOut:6000];
        sleep(0.2);
        int speed = [[fixture WriteReadString:fan_get atSite:site-1 timeOut:6000] intValue];
        if (speed > 0)
        {
            is_ok = true;
            break;
    ...
    if (pre_speed >= 100) { pre_speed = 100; }
    if (pre_speed <= 0) { pre_speed = 0; }
    
    NSString *fan_setback = [NSString stringWithFormat:@"%@(4000,%d)",[cmd objectForKey:kFIXTUREFANSPEEDSET],100-pre_speed];
    [fixture WriteReadString:fan_setback atSite:site-1 timeOut:6000];
    writeFixtureLog([NSString stringWithFormat:@"[fan set back cmd] %@, [result] %d\r\n",fan_setback,is_ok]);
    return is_ok;
}
