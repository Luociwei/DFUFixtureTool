...
def test_register_rpc_service(rpc_client_wrapper, rpc_server_wrapper, rpc_service_list):
    ...
    assert "Name set(['exception_test', 'args_test', 'sleep', 'non_utf8_str', 'measure']) already registered in subdispather dummy_" in str(e)
    with pytest.raises(RPCError) as e:
...
def test_serialize_failure(rpc_client_wrapper, rpc_server_wrapper):
    ...
    # threadpool tasks number shall be zero;
