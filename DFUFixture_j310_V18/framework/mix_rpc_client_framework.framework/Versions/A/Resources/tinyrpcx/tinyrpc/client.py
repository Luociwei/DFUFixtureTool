...
class RPCClient(object):
    ...
    def send_file(self, src_file, dst_folder, timeout_ms=60 * 1000.0):
        ...
        '''
        if not src_file:
            return 'Source file {} invalid'.format(src_file)
        if not os.path.isfile(src_file):
            return 'Source file {} is not accessible as a file'.format(src_file)
        ...
        return self.get_proxy().server_send_file(dst_fn, data, dst_folder, timeout_ms=timeout_ms)
    ...
    def get_file(self, target, timeout_ms=60 * 1000.0):
        ...
        ret, data = self.get_proxy().server_get_file(target, timeout_ms=timeout_ms)
        return ret, base64.b64decode(data)

    def get_and_write_all_log(self, timeout_ms=60 * 1000.0):
        '''
        ...
        :return: tuple as below:
                ...
                (err_msg, '') when fail; errmsg is string; '' is empty string.
        '''
        print 'Getting log from xavier with {:02f}s timeout'.format(timeout_ms / 1000.0)

        ret, data = self.get_proxy().xavier_get_all_log(timeout_ms=timeout_ms)
        if ret != 'PASS':
            return ret

        with open('rpc_log.tgz', 'wb') as f:
